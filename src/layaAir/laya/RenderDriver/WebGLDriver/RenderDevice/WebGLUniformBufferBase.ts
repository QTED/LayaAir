...
export abstract class WebGLUniformBufferBase {
    ...
    setMatrix3x3(index: number, value: Float32Array) {
        let uniform = this.descriptor.uniforms.get(index);
        if (uniform) {
            for (let i = 0; i < 3; i++) {
                for (let j = 0; j < 3; j++) {
                    uniform.view[i * 4 + j] = value[i * 3 + j];
                }
            }
    ...
    setMatrix4x4(index: number, value: Float32Array) {
        let uniform = this.descriptor.uniforms.get(index);
        if (uniform) {
            ...
            switch (type) {
                ...
                case ShaderDataType.Matrix3x3:
                    if (uniform.arrayLength > 0) {
                        ...
                        this.setMatrix3x3(index, (data as Matrix3x3).elements);
                    }
                    break;
                case ShaderDataType.Matrix4x4:
                    if (uniform.arrayLength > 0) {
                        ...
                        this.setMatrix4x4(index, (data as Matrix4x4).elements);
                    }
                    break;
